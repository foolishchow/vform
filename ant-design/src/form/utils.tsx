...
import type {
  DeepKey, DynamicDef, Into,
  VPropDef, VTransfer, _DynamicDef
} from './types'

export function mergeProps<FormData, Props>(
  data: FormData,
  props?: VPropDef<FormData, Props>
): Partial<Props> {
  if (!props) return {}
...
export function wrapDynamic<T, Option>(
  def: { type: string, config: DynamicDef<T, Option> }
) {
...
export function isDynamic<T, Option>(
  def: DynamicDef<T, Option>
): def is _DynamicDef<T, Option> {
  return typeof def == 'function'
}
export function getDynamic<T, Option>(
  form: T, def?: DynamicDef<T, Option> | undefined
): Option | undefined {
...
export function getWithTransfer<
  T extends object,
  Key extends DeepKey<T>
>(
...
export function setWithTransfer<
  T extends object,
  Key extends DeepKey<T>
>(

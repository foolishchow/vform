...
export type BuildTypes = Exclude<
  keyof VFormItemTypes & string,
  'build' | 'merge' | 'form'
>
...
interface Builder_Items<
  Type extends BuildTypes,
  T extends object,
  Configed extends object = {}
> {
  <Key extends Exclude<DeepKey<T>, keyof Configed> = never>(
    config: VFormItemTypes<T, Key>[Type]
      | _DynamicDef<T, VFormItemTypes<T, Key>[Type]>
...
interface Builder_Merger<
  T extends object, Configed = {}
> {
  <U extends object, _Configed extends object>(
    builder: Builder<U, _Configed>
  ): MergeResult<T, Configed, _Configed>
}

type Builder_Methods = 'form' | 'build' | 'merge' | BuildTypes

type BuilderInstance<
  Type extends Builder_Methods,
  T extends object,
  Configed extends object = {}
> =
